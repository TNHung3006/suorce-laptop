...
CREATE PROCEDURE ThongKeTongGioHoanThanh
AS
BEGIN
    -- Tạo bảng tạm để lưu trữ MaDA của các dự án đã hoàn thành
    DROP TABLE IF EXISTS #DuAnHoanThanh;
    CREATE TABLE #DuAnHoanThanh (
        MaDA VARCHAR(10) PRIMARY KEY
    );

    -- Lấy MaDA của các dự án mà tất cả công việc đều đã hoàn thành
    INSERT INTO #DuAnHoanThanh (MaDA)
    SELECT DISTINCT
        da.MaDA
    FROM
        DuAn da
    WHERE NOT EXISTS (
        SELECT 1
        FROM CongViec cv
        WHERE cv.MaDA = da.MaDA AND cv.TrangThai <> 'Hoàn thành'
    );

    -- Tính tổng số giờ làm việc cho các dự án đã hoàn thành
    SELECT
        COUNT(DISTINCT daht.MaDA) AS SoLuongDuAnHoanThanh,
        SUM(pc.SoGioThamGia) AS TongSoGioLamViecDuAnHoanThanh
    FROM
        #DuAnHoanThanh daht
    JOIN
        CongViec cv ON daht.MaDA = cv.MaDA
    JOIN
        PhanCong pc ON cv.MaCV = pc.MaCV;
...
BEGIN
    ...
    BEGIN
        IF EXISTS (
            SELECT 1
            FROM inserted i
            JOIN DuAn d ON i.MaDA = d.MaDA
            WHERE i.KinhPhi > d.KinhPhiBanDau
        )
        BEGIN
            RAISERROR('Kinh phí dự án không được vượt quá ngân sách ban đầu.', 16, 1);
